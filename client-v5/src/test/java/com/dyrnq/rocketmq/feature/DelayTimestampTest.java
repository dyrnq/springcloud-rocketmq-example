...
/**
 * Verifies a message delivered at a future timestamp via {@code setDeliveryTimestamp(now + delay)}
 * on the V5 gRPC client.
 *
 * <p>Replaces the body of {@code
 * org.apache.rocketmq.client.java.example.ProducerDelayMessageExample.main}.
 */
...
class DelayTimestampTest {
            ...
            .setMessageListener(
                (MessageView messageView) -> {
                  ...
                  if (body.equals(bodyMarker)) {
                    log.info(
                        "Received matching delayed message, id={}", messageView.getMessageId());
  ...
  void sendDelayedMessageArrivesAfterDelay() throws Exception {
    ...
    assertTrue(
        deliveryAt > 0,
        "Delayed message should have been delivered within " + UPPER_BOUND_MS + "ms");
    long elapsed = deliveryAt - sentAt;
    log.info("Elapsed: {}ms", elapsed);
    assertTrue(
        elapsed >= LOWER_BOUND_MS,
