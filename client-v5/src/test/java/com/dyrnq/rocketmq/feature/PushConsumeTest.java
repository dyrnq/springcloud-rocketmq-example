...
/**
 * Verifies that a V5 push consumer subscribes to {@link Topics#TOPIC_A} and receives a message sent
 * to it. Uses {@code Addresses.PROXY} (8181) — the original {@code PushConsumerExample} mistakenly
 * pointed at 9876 (the namesrv port); this test reuses the proxy endpoint and the test documents
 * the fix.
 *
 * <p>Replaces the body of {@code org.apache.rocketmq.client.java.example.PushConsumerExample.main}
 * (with the 9876→8181 port bug corrected).
 */
...
class PushConsumeTest {
        ...
        provider
            ...
            .setSubscriptionExpressions(
                Collections.singletonMap(
                    Topics.TOPIC_A,
                    new FilterExpression("yourMessageTagA", FilterExpressionType.TAG)))
            .setMessageListener(
  ...
  void receivesOneMessageAfterProducerSends() throws Exception {
    ...
    assertTrue(
        matchingBodies.contains(bodyMarker),
