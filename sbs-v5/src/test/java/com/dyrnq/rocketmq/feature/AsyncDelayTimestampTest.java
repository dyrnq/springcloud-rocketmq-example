...
/**
 * Verifies async scheduled/delay send via
 * {@code RocketMQClientTemplate.asyncSendDelayMessage} (the V5 client
 * has no fixed delay-level API, so V5 tests are timestamp-based). This
 * complements the sync variant in
 * {@code com.dyrnq.rocketmq.feature.DelayTimestampTest} by exercising the
 * async path that {@code ProducerRunner} also uses.
 */
...
class AsyncDelayTimestampTest {
    ...
    void asyncSendDelayMessage() throws Exception {
        ...
        rocketMQClientTemplate.asyncSendDelayMessage(Topics.TOPIC_DELAY,
            "sbs-v5-async-delay".getBytes(StandardCharsets.UTF_8),
            Duration.ofSeconds(10), future);
