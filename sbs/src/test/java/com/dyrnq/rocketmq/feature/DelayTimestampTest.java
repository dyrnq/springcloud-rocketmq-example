...
/**
 * Verifies a V4 scheduled message (deliver at absolute timestamp = now + 10s) sent via a raw {@link
 * DefaultMQProducer} is delivered to a push consumer on {@link Topics#TOPIC_DELAY} at least 9s
 * after send time. V4 uses {@code Message.setDeliverTimeMs} (epoch millis) for scheduled delivery.
 *
...
class DelayTimestampTest {
  ...
  void receivesDelayedMessageAfterTimestamp() throws Exception {
    ...
    assertTrue(
        elapsed >= DELAY_MS - DELAY_TOLERANCE_MS,
