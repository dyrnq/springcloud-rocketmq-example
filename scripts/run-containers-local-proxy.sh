...
next_grpcServerPort=$((grpcServerPort+1000*i))
next_remotingListenPort=$((remotingListenPort+1000*i))
sed \
...
echo "}"
) > $HOME/$name-rmq-proxy.json
cat < $HOME/$name-rmq-proxy.json
...
-v $HOME/$name-rmq-proxy.json:/opt/rocketmq/conf/rmq-proxy.json \
...
--network mynet \
${rocketmq_image} mqbroker -c /etc/rocketmq/broker.conf --enable-proxy
done
}
