...
/**
 * Verifies the Spring Cloud Stream binder end-to-end: send a message via {@link StreamBridge} and
 * assert a raw V4 push consumer on the same topic ({@code demo}) receives it. The function-bean
 * wiring ({@code StreamConsumer.consumerEvent}) is exercised by the {@code @SpringBootTest} context
 * boot and is not directly asserted here — this test is the same pattern as {@code
 * com.dyrnq.rocketmq.feature.PushConsumeTest} in the {@code client} module.
 *
 * <p>Replaces the body of the original {@code com.dyrnq.sca.rocketmq.StreamConsumerRunner.run} (the
 * raw V4 push consumer) for spring-cloud-v2021.
 */
