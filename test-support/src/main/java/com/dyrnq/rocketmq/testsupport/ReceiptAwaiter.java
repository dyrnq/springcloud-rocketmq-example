...
/**
 * Helper that polls an {@link AtomicInteger} (typically a counter incremented by a consumer
 * callback) until it reaches a target value, or throws {@link AssertionError} on timeout.
 *
 * <p>Lightweight re-implementation of awaitility's {@code await().until(callable)} that avoids
 * pulling in the awaitility dependency on modules that don't otherwise use it.
 */
